package io.castle.client.deprecated.http;
...
import io.castle.client.deprecated.objects.Error;
import io.castle.client.deprecated.objects.Session;
import io.castle.client.deprecated.objects.UserInfoHeader;
...
public class HttpClient {
    ...
    private static final String USER_AGENT = CastleDeprecated.USER_AGENT;
    ...
    private final HttpConnectorSupplier connection = CastleDeprecated.getHttpConnectorSupplier();
    ...
    private <T> T handleError(URI uri, HttpURLConnection conn, int responseCode) throws IOException {
        return throwException(readEntity(conn, responseCode, new TypeReference<io.castle.client.deprecated.objects.Error>() {
        }), responseCode, uri);
    }

    private <T> T handleSuccess(TypeReference<T> response, HttpURLConnection conn, int responseCode) throws IOException {
        String updatedToken = conn.getHeaderField("X-CastleDeprecated-Set-Session-Token");
    ...
    private HttpURLConnection prepareConnection(HttpURLConnection conn) {
        conn.setConnectTimeout(CastleDeprecated.getConnectionTimeout());
        conn.setReadTimeout(CastleDeprecated.getRequestTimeout());
        conn.setUseCaches(false);
        return conn;
    }

    private Map<String, String> createAuthorizationHeaders() {
        String authString = ":" + CastleDeprecated.getAPISecret();
    ...
    private Map<String, String> createHeaders() {
        ...
        headers.put("X-CastleDeprecated-Client-User-Agent", CLIENT_AGENT_DETAILS);

        headers.put("Accept-Charset", UTF_8);
        headers.put("Accept", APPLICATION_JSON);
        if (CastleDeprecated.isDisableTracking()) {
            headers.put("X-CastleDeprecated-Do-Not-Track", "1");
        }
        if (this.session != null) {
            headers.put("X-CastleDeprecated-Session-Token", this.session.getToken());
        }
        if (this.info != null) {
            headers.put("X-CastleDeprecated-Ip", this.info.getIp());
            if(this.info.getClientId()==null) {
                headers.put("X-CastleDeprecated-Client-Id", "");
            }else{
                headers.put("X-CastleDeprecated-Client-Id", this.info.getClientId());
            }
            headers.put("X-CastleDeprecated-Headers", this.info.getHeaders());
        }
        return headers;
    }
